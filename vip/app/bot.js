...
client.on("ready", () => {
  client.channels.cache.get("970439560184094760").join();   
    ...
    if (message.channel.type === "dm") return;
      if(message.author.bot) return;  
        if (message.content.length > 4) {
         if (db.fetch(`guardcl_${839309996100681749}`)) {
...
client.on("message", async msg => {
  ...
  let a = await db.fetch(`kufur_${msg.guild.id}`)
    if (a == 'acik') {
      const küfür = [
        "yarak","mk", "amk", "aq", "orospu", "Oruspu", "oç", "sikerim", "yarrak", "piç", "amq", "sik", "amcık", "çocu", "sex", "seks", "amına", "orospu çocuğu", "sg", "siktir git","ebenin","sikerim","sik","sikiyim","Amk","Mq","SİKTİR","OROSPU","SİKİK","YARRAM","YARRAK","PİÇ","SİK","SİKERİM","AMK","AM","SG","MQ","MAL","Amq","siq","s2iş","s2ik","Phiç","phiç","Piç","fuck","ananı sikerim","ananı","babanı","allahını"
                  ]
          ...
          try {
            if (!msg.member.hasPermission("ADMINISTRATOR")) {
                  msg.delete();
                    ...
                    return msg.channel.send(`<:bangbang:959130940498653194> **Küfür Etmek Yasak !!!**`).then(msg => msg.delete({ timeout: 5000}));
            }          
                } catch(err) {
                  console.log(err);
                }
...
client.on('message', async (message) => {
    if (message.content.startsWith(`${prefix}yardım`)) {
        ...
        .setStyle('grey')
        .setEmoji('<:onay:959787747546112040>') 
        .setID('genel') 
        ...
        message.channel.send('', {
          ...
          .setColor(`random`)  
          .setDescription(`**Jail Komutları : <:guard1:959514100331868241> Butonuna Tıklayınız\nBan Komutları : <:guard2:959514100327645193> Butonuna Tıklayınız\nKick Komutları : <:ayar:959130474071093248> Butonuna Tıklayınız\nGenel Komutlar : <:onay:959787747546112040> Butonuna Tklayınız\nAna Menü : <:anamenu:958825699815338034> Butonuna Tıklayınız**\n\n**${message.author.tag} - Tarafından İstendi**`),
            buttons:[
...
.setFooter(`${ayarlar.botisim}`, client.user.avatarURL())
    ...
    const embed = new Discord.MessageEmbed()
.setDescription(`***Ana Menü***\n\n**Jail Komutları : <:guard1:959514100331868241> Butonuna Tıklayınız\nBan Komutları : <:guard2:959514100327645193> Butonuna Tıklayınız\nKick Komutları : <:ayar:959130474071093248> Butonuna Tıklayınız\nGenel Komutlar : <:onay:959787747546112040> Butonuna Tklayınız\nAna Menü : <:anamenu:958825699815338034> Butonuna Tıklayınız**`)
.setFooter(`${ayarlar.botisim}`, client.user.avatarURL())
    button.message.edit(embed)
  }
    ...
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('KICK_MEMBERS')) {
          ...
          return message.reply('<:yha:959130940498653194> **Reklam Yasak!**').then(message => message.delete(6000));
